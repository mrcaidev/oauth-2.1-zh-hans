...
授权端点 URL **禁止**包含片段部分，**可以**包含查询字符串部分（[附录 C.1](../13-appendices/03-serializations.md#c1-查询字符串序列化)）。后续添加额外的查询参数时，**必须**保留原查询部分。
...
在重定向可能携带用户凭据的请求时，授权服务器**必须**避免意外转发这些用户凭据（详见[第 7.5.4 节](../07-security-considerations/05-authorization-code-security-considerations.md#754-http-307-重定向)）。
